...
async def _run_import_pipeline(parsed: Dict[str, Any], title: str, tags: str) -> Dict[str, Any]:
    """
...
async def import_pdf(file_path: str, title: str = "", tags: str = "") -> Dict[str, Any]:
    """导入 PDF 文档到知识库。Args: file_path, title, tags."""
    ...
    return await _run_import_pipeline(await parser.parse(file_path), title, tags)
...
async def import_pptx(file_path: str, title: str = "", tags: str = "") -> Dict[str, Any]:
    """导入 PPTX 演示文稿到知识库。Args: file_path, title, tags."""
...
async def import_docx(file_path: str, title: str = "", tags: str = "") -> Dict[str, Any]:
    """导入 DOCX Word 文档到知识库。Args: file_path, title, tags."""
...
async def import_webpage(url: str, title: str = "", tags: str = "") -> Dict[str, Any]:
    """导入网页为 Markdown 笔记。Args: url, title, tags."""
    ...
    return await _run_import_pipeline(await parser.parse(url), title, tags)
...
async def import_document(source: str, title: str = "", tags: str = "") -> Dict[str, Any]:
    """智能导入文档（自动识别类型）。Args: source（文件路径或 URL）, title, tags."""
    from core.file_parser import DocumentParserFactory
    source = source.strip().strip('"').strip("'")
    parsed = await DocumentParserFactory().parse(source)
    return await _run_import_pipeline(parsed, title, tags)


@YA_MCPServer_Tool(
    name="batch_import_documents",
    title="Batch Import Documents",
    description="批量导入整个文件夹内的文档（PDF、PPTX、DOCX）到知识库。支持递归子目录，可按文件类型过滤",
)
async def batch_import_documents(
    ...
    for f in files:
        try:
            ...
            r = await _run_import_pipeline(parsed, title="", tags=tags)
